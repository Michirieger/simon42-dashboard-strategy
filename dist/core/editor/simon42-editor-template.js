...
export function renderAreaEntitiesHTML(areaId, groupedEntities, hiddenEntities) {
  ...
  const sensorSelectorsHTML = `
    <div class="area-sensor-settings" style="padding: 12px; background: var(--secondary-background-color); border-bottom: 1px solid var(--divider-color); margin-bottom: 12px;">
      ...
      <div style="font-size: 0.8em; opacity: 0.6; margin-top: 4px;">Konfiguriere hier die Sensoren, die in der Übersicht dieses Bereiches angezeigt werden sollen. Wähle "Automatisch", um den Standard beizubehalten. Beachte, dass wenn mehrere gleiche Sensoren (z.B. Temperatur) in einem Bereich sind, der Durchschnitt aller Sensoren angezeigt wird.</div>
    </div>
  `;
...
function renderSensorSelectors(areaId) {
  ...
  // filter entitys that are displayed in the dropdown list
  const isSensorType = (e, type) => {
    if (!e.entity_id.startsWith('sensor.')) return false;

    const regEntry = globalHass && globalHass.entities && globalHass.entities[e.entity_id];
    if (regEntry) {
        // filter system entitys as the areaCard will not display them
        if (regEntry.entity_category === 'diagnostic' || regEntry.entity_category === 'config') {
            return false;
        }
    }
